...
- [x] Componentes M3: Search con chips (Filter/Assist -> ActionChip), TextField filled; Tracker con `ListTile` M3 + tonal; Config/Comunidad con `Card` + `ListTile` y chips de acción.
- [x] Shapes y spacing: bordes 12/16px en `cardTheme/bottomSheet/dialog`, mantener escala AppSpacing (4/8/12/16/24); revisar maxWidth y grids ya existentes.
- [x] Unificar transiciones M3 también en modales/diálogos (`modalBottomSheetTheme` con drag handle y `dialogTheme` alineado) + helper `showAppModalSheet`.
